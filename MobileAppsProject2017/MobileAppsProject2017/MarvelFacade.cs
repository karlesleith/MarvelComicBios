...
namespace MobileAppsProject2017
{
    public class MarvelFacade
    {
        ...
        private int characterNoId = 1009610;
        ...
        public static async Task PopulateMarvelCharactersAsync(ObservableCollection<Character> marvelCharacter)
        {
            ...
            {
                //filter characters miss thumbnail
                if (character.thumbnail != null && character.description != null)
                {
        ...
        private static async Task<CharacterDataWrapper> GetCharacterDataWrapperAsync()
        {
            ...
            Debug.WriteLine("OUTLOG: ", result);
        }

        private static async Task<ComicDataWrapper> GetComicDataWrapperAsync(int characterNoId)
        {
            // Assemble the URL
            ...
            string url = String.Format("https://gateway.marvel.com:443/v1/public/comics?characters={0}&limit=10&apikey={1}&ts={2}&hash={3}", characterNoId, PublicKey, timeStamp, hash);
        ...
        public static async Task PopulateMarvelComicsAsync(int characterNoId, ObservableCollection<ComicBook> marvelComic)
        {
            try
            {
                var comicDataWrapper = await GetComicDataWrapperAsync(characterNoId);

                var comics = comicDataWrapper.data.results;

                foreach (var comic in marvelComic)
                {
